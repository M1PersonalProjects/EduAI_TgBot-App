...
EDUCATIONAL_PATTERNS: Sequence[re.Pattern[str]] = (
    ...
    re.compile(
        r"\b(математик|алгебр|геометр|физик|хими|биологи|истори|географ|русск|литератур|информатик|программирован|дифференциал|корень|таблиц[аы] менделеев|план урока)\w*\b",
        re.IGNORECASE,
    ),
)
...
async def validate_request_scope(
        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    "Если учебник не выбран, разрешены любые образовательные вопросы "
                    "по школьным и базовым университетским дисциплинам, методике обучения, "
                    "составлению плана урока, информатике и программированию. В этом режиме "
                    "matches_selected_context установи в true для любого образовательного запроса. "
                    "Запрещены бытовые, развлекательные, новостные, спортивные и иные неучебные запросы."
                ),
            },
            {
                "role": "user",
    ...
    allowed = bool(
        ...
        and (context is None or classification.matches_selected_context)
    )
