...
Основная регистрация выполняется через Telegram-бота Umnix. После регистрации WebApp может авторизовать пользователя через Telegram WebApp. В обычном браузере доступен вход по Telegram ID для существующей учётной записи.
